# RightNow CLI Installation Script for Windows
...
function Show-Banner {
    ...
    Write-Host "║        RightNow CLI Installer                  ║" -ForegroundColor Cyan
...
function Install-ViaPip {
    ...
    Write-Info "Installing RightNow CLI via pip..."
    ...
    Write-Info "Installing rightnow-cli package..."
    try {
        & $PythonCmd -m pip install --user rightnow-cli
        if ($LASTEXITCODE -ne 0) {
            throw "Installation failed"
        }
        Write-Success "RightNow CLI installed successfully"
    } catch {
...
function Install-FromSource {
    ...
    Write-Info "Installing RightNow CLI from source..."
    ...
    $tempDir = Join-Path $env:TEMP "rightnow-cli-install-$(Get-Random)"
    ...
    try {
        ...
        Write-Success "RightNow CLI installed from source"
    } catch {
        Write-Error "Installation failed: $_"
        exit 1
    } finally {
...
function Install-InVenv {
    ...
    $venvPath = Join-Path $env:USERPROFILE ".rightnow-cli"

    Write-Info "Installing RightNow CLI in virtual environment..."
    ...
    & python -m pip install rightnow-cli | Out-Null

    # Create wrapper batch file
    $wrapperPath = Join-Path $env:USERPROFILE "AppData\Local\Microsoft\WindowsApps\rightnow.bat"
    $wrapperContent = @"
@echo off
call "$venvPath\Scripts\activate.bat"
python -m rightnow_cli %*
"@

    New-Item -ItemType Directory -Path (Split-Path $wrapperPath) -Force -ErrorAction SilentlyContinue | Out-Null
    Set-Content -Path $wrapperPath -Value $wrapperContent

    Write-Success "RightNow CLI installed in virtual environment"
...
function Show-PostInstall {
    ...
    Write-Host "rightnow" -ForegroundColor Yellow
    Write-Host "  3. Paste your API key when prompted"
    Write-Host ""
    Write-Host "Commands:" -ForegroundColor Cyan
    Write-Host "  rightnow          - Start the CLI"
    Write-Host "  rightnow --help   - Show help"
    Write-Host ""
    Write-Host "Documentation:" -ForegroundColor Cyan
    Write-Host "  https://github.com/$REPO" -ForegroundColor Blue
    Write-Host ""

    # Test if rightnow command works
    try {
        $testCmd = & rightnow --version 2>&1
        if ($LASTEXITCODE -eq 0) {
            Write-Success "RightNow CLI is ready to use!"
        }
    } catch {
        Write-Warning "You may need to restart your terminal for the 'rightnow' command to work"
    }
}
