#!/bin/bash
# RightNow CLI Installation Script
# This script installs RightNow CLI on Linux and macOS
...
install_via_pip() {
    print_info "Installing RightNow CLI via pip..."

    # Ensure pip is up to date
    $PYTHON_CMD -m pip install --upgrade pip

    # Install rightnow-cli
    if $PYTHON_CMD -m pip install --user rightnow-cli; then
        print_success "RightNow CLI installed successfully"
...
install_from_source() {
    print_info "Installing RightNow CLI from source..."
    ...
    if git clone https://github.com/$REPO.git; then
        ...
        print_success "RightNow CLI installed from source"
    else
...
install_in_venv() {
    print_info "Installing RightNow CLI in virtual environment..."
    ...
    print_info "Installing RightNow CLI..."
    pip install --upgrade pip
    pip install rightnow-cli

    # Create wrapper script
    mkdir -p "$INSTALL_DIR"
    cat > "$INSTALL_DIR/rightnow" << EOF
#!/bin/bash
source "$VENV_DIR/bin/activate"
exec rightnow "\$@"
EOF

    chmod +x "$INSTALL_DIR/rightnow"

    print_success "RightNow CLI installed in virtual environment"
    print_info "Wrapper script created at $INSTALL_DIR/rightnow"
...
post_install() {
    ...
    echo "  rightnow          - Start the CLI"
    echo "  rightnow --help   - Show help"
    echo ""
